...
const CreateConstitution = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const response = await privateAxios.post(`/private/create-constitution`, {
        title,
        body,
        published,
        groupId,
      ...
      if (response.status === 201) {
        ...
        setNotification({
          visible: true,
          type: "success",
          message: response.data.message || "Constitution created successfully.",
        });
        setTimeout(() => {
      ...
      setNotification({
        ...
        type: "error",
        message:
          error.response?.data?.message || "Failed to create constitution.",
      });
      setTimeout(() => setNotification({ visible: false, type: "", message: "" }), 3000);
...
export default CreateConstitution;