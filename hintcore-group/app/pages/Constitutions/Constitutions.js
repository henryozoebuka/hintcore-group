...
const Constitutions = ({ navigation }) => {
    ...
    const fetchConstitutions = async (pageNumber = 1) => {
        try {
            ...
            let list = response.data.constitutions || [];

            setConstitutions(list);
            setTotalPages(response.data.totalPages || 1);
            setCurrentPage(pageNumber);
        } catch (error) {
            showError(error?.response?.data?.message || "Failed to fetch constitutions.");
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <View style={[stylesConfig.SETTINGS_STYLES.container, { backgroundColor: colors.background }]}>
                ...
                <Text style={[stylesConfig.SETTINGS_STYLES.header, { color: colors.text }]}>Constitutions</Text>
                ...
                {initialLoaded &&
                    ...
                    >
                        <Text style={{ color: colors.mainButtonText }}>
                            {searchOptions ? "Hide Search Options" : "Filter Constitutions"}
                        </Text>
                    </Pressable>}
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <Pressable
                                ...
                                onPress={() => navigation.navigate("constitutions", { id: item._id })}
                            >
