...
const Announcements = ({ navigation }) => {
    ...
    const fetchAnnouncements = async (pageNumber = 1) => {
        try {
            ...
            let list = response.data.announcements || [];

            setAnnouncements(list);
            setTotalPages(response.data.totalPages || 1);
            setCurrentPage(pageNumber);
        } catch (error) {
            showError(error?.response?.data?.message || "Failed to fetch announcements.");
    ...
    return (
        <SafeAreaView style={{ flex: 1 }}>
            <View style={[stylesConfig.SETTINGS_STYLES.container, { backgroundColor: colors.background }]}>
                ...
                <Text style={[stylesConfig.SETTINGS_STYLES.header, { color: colors.text }]}>Announcements</Text>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <Pressable
                                ...
                                onPress={() => navigation.navigate("announcement", { id: item._id })}
                            >
