...
const ManageEditMinutes = ({ route, navigation }) => {
    ...
    // Fetch the minutes data to pre-fill the form
    useEffect(() => {
        const fetchMinutes = async () => {
            try {
                ...
                if (response.status === 200) {
                    ...
                    setPublished(minutes.published);
                } else {
                ...
                setNotification({
                    visible: true,
                    type: 'error',
                    message: 'Failed to fetch minutes details.',
                });
    ...
    const handleSubmit = async () => {
        ...
        try {
            setLoading(true);
            const response = await privateAxios.patch(`/private/update-minutes/${id}`, { title, body, published });
            ...
            setNotification({
                visible: true,
                type: 'error',
                message: error.response?.data?.message || 'Failed to update minutes.',
            });
            setTimeout(() => {
                setNotification({ visible: false, type: '', message: '' });
            }, 3000);
...
export default ManageEditMinutes;